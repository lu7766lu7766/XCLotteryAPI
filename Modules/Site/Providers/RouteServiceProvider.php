...
class RouteServiceProvider extends ServiceProvider
{
    ...
    {
        $file = \Module::getModulePath('Site') . '/Routes/web.php';
        if (file_exists($file)) {
    ...
    {
        $file = \Module::getModulePath('Site') . '/Routes/api.php';
        if (file_exists($file)) {
            \Route::prefix('api')
                ->middleware('api')
                ->group($file);
    ...
    {
        $folder = \Module::getModulePath('Site') . '/Routes/custom';
        if (\File::exists($folder)) {
            foreach (\File::files($folder) as $file) {
                \Route::as('site_')->group($file);
            }
        }
    }
}
